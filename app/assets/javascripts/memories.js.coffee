	...
	loadMap: ->
		...
		google.maps.event.addListener map, 'dblclick', (event) ->
			...
			$('#story_photo_proxies_').change ->
				...
				$.each @files, (index, file) -> $('#fileinput-list').append $('<li />').html(file.name)
		...
		@memories.forEach (memory) -> window.Memories.loadMarker map, memory.lat, memory.lng, memory.body, memory.storyId

	loadMarker: (map, lat, lng, body, storyId) ->
